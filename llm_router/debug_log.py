"""Structured debug logging for LLM Router."""
...
DEBUG_LOG_FILE: str = "llm_router.jsonl"
...
def log_debug(event: str, data: dict[str, Any] | None = None, truncate: bool = True) -> None:
    """Log a structured debug event as one JSON object per line.

    Args:
        event: Short event name describing the entry.
    ...
    log_data = _truncate_large_content(data) if (data and truncate) else data
    entry = {
        "ts": datetime.now().isoformat(timespec="seconds"),
        "event": event,
        "data": log_data,
    }
    _LOG_FILE_HANDLE.write(json.dumps(entry, ensure_ascii=False))
