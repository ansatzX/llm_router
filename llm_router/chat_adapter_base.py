"""Shared Chat Completions adapter helpers.

Provider Chat APIs commonly accept Chat function tools while Codex sends Responses-shaped input and tools. This module contains provider-neutral Responses-to-Chat conversion helpers.
"""
...
class ChatCompletionAdapterBase:
    """Provider-neutral Responses and Chat conversion base."""
    ...
    def load_provider_state(self, provider_state: dict[str, Any] | None) -> None:
        """Load persisted provider-private state for one Responses session."""
        self.reset()
    ...
    def dump_provider_state(self) -> dict[str, dict[str, str]]:
        """Return provider-private state suitable for session persistence."""
        return {
            "reasoning_by_call_id": dict(self.reasoning_by_call_id),
        }

    def filter_request_payload(self, payload: dict[str, Any]) -> dict[str, Any]:
        """Keep only request fields the provider Chat Completion API accepts."""
        ...
        if dropped:
            log_debug("CHAT_ADAPTER_CHAT_PAYLOAD_FILTER", {
    ...
    def flatten_response_items(self, messages: list[Any]) -> list[dict[str, Any]]:
        """Flatten Responses items into Chat messages.
        ...
        for msg in messages:
            ...
            if isinstance(converted, list):
                ...
                result.append(converted)
        flush_pending_tool_calls()
        log_debug("CHAT_ADAPTER_RESPONSES_TO_CHAT_CONVERSION", {
    ...
    def responses_tools_to_chat(
        ...
        """Convert all Codex Responses tools to Chat function tools."""
        converted = []
        for tool in tools:
        ...
        log_debug("CHAT_ADAPTER_CHAT_TOOL_CONVERSION", {
            "input_tools": [
    ...
    def namespace_tool_to_chat(self, tool: dict[str, Any]) -> list[dict[str, Any]]:
        """Expand a Responses namespace into Chat function tools."""
    ...
    def chat_response_to_output_items(
        ...
        """Convert a Chat Chat assistant message into Responses output.

        Chat returns native Chat ``tool_calls`` even for Codex Responses
